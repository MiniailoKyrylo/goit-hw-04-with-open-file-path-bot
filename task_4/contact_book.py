...
def add_contact(path: str = 'task_4/contacts.txt', name: str = '', phone_number:str = '') -> None:
    ...
    if not phone_number_standart[0]:
        output(f'ПОМИЛКА (add_contact) -> Контакт не створено: "{phone_number}" - не може бути номером телефону', 'r')
        output(f'УВАГА -> Номер телефону має містити від 10 до 12 цифр.', 'y')
    ...
    # Перевіряємо чи не існує такого номера у наших контактах та друкуємо повідомлення що контакт не збережено, бо такий номер телефону вже зайнятий
        ...
        if phone_number_find[0]:
            output(f'ПОМИЛКА (add_contact) -> Контакт не створено: "{phone_number_standart[1]}" вже використовується контактом {phone_number_find[1][phone_number_standart[1]]}', 'r')
        ...
        # Виконуємо дадовання номеру телефону до списку контактів
            ...
            output(f'ВИКОНАНО (add_contact) -> Контакт створено: {phone_number_standart[1]} - {name}', 'g') # Друкуємо повідомлення що контакт збережено
...
def open_read_file(path: str = 'task_4/contacts.txt', flag: str = 'r', text: str = '') -> str:
    ...
    try:            
        ...
        output("ПОМИЛКА (open_read_file) -> Файл не знайдено.", 'r')
    except IOError:
        output("ПОМИЛКА (open_read_file) - > Неможливо прочитати або записати данні.", 'r')
    except Exception as e:
        output(f'ПОМИЛКА (open_read_file) -> {e}', 'r')
    finally:
...
def convert_str_dict(text) -> dict:
    ...
    for item in text.split('\n'):
        ...
        try:
            ...
            output("ПОМИЛКА (convert_str_dict) -> Рядок не конверовано: не коректний вхідний рядок", 'r')
            all_contacts_dict = {}
            break
        except Exception as e:
...
def edit_contact(path: str = 'task_4/contacts.txt', find_value: str = '') -> None:
    ...
    if not result_find[0]:
        output('УВАГА -> Контакта за Вашим запитом не знайдено.', 'y')
    ...
    if result_find[0]:
        ...
        if replace == 'name':
            ...
            if not phone_number_standart[0]:
                output(f'ПОМИЛКА (edit_contact) -> Контакт не створено: "{phone_number}" - не може бути номером телефону', 'r')
                output(f'ПІДКАЗКА -> Номер телефону має містити від 10 до 12 цифр.', 'y')
                ...
                if phone_number_find[0]:
                    output(f'ПОМИЛКА (edit_contact) -> Контакт не створено: "{phone_number_standart[1]}" вже використовується контактом {phone_number_find[1][phone_number_standart[1]]}', 'r')
                else:
        ...
        if check_all_if:
            ...
            output(f'ВИКОНАНО (edit_contact) -> Контакт змінено: {new_phone_number_and_name}', 'g')
...
def display_contacts(path: str = 'task_4/contacts.txt') -> None:
    ...
    if all_contacts:
        ...
        output('УВАГА: Телефона книга не містить контактів або пошкоджена.', 'y')
